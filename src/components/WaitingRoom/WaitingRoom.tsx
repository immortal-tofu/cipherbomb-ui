...
export const WaitingRoom = ({ contract, account }: WaitingRoomProps) => {
  ...
  useEffect(() => {
    if (contract) {
      ...
      // const jsonProvider = new JsonRpcProvider('https://devnet.zama.ai');
      const gameContract = contract.connect(jsonProvider);
      // void gameContract.on(gameContract.filters.PlayerJoined, refreshPlayers);
      // void gameContract.on(gameContract.filters.PlayerKicked, refreshPlayers);
      void gameContract.on(gameContract.filters.PlayerNameChanged, onPlayerNameChanged);
      return () => {
        console.log('off');
        // void gameContract.off(gameContract.filters.PlayerJoined, refreshPlayers);
        // void gameContract.off(gameContract.filters.PlayerKicked, refreshPlayers);
